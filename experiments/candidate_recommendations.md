...
2. `submissions/22_loo_spatial_mild_nongal.csv`
   - Local OOF: n/a; final-only LOO spatial train/test mismatch candidate.
   - Public LB: n/a.
   - Rationale: extends the public-winning `19` direction with fewer GALAXY predictions (`578` rows changed vs `16`, GALAXY count `-443`). Submit next; `23` tied `19`, so STAR tilt did not add lift.

3. `submissions/24_loo_spatial_stronger_nongal.csv`
   - Local OOF: n/a; final-only LOO spatial train/test mismatch candidate.
   - Public LB: n/a.
...
- If `22` clears `0.97`, update `experiments/leaderboard.md`, `PROGRESS.md`, and `DECISIONS.md` before any further modeling.
