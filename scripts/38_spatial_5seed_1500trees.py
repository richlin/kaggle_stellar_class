"""5-seed + 1500-tree spatial LGBM blend (combines script 36 results with 2 extra seeds).

Reuses the 3-seed 1500-tree OOF/test probs from script 36, then trains
seeds 45 and 46 with n_estimators=1500 and averages all 5 seeds.
Blends with the spatial XGBoost from script 16.

Requires scripts/36_spatial_more_trees.py to have been run first.
Acceptance gate: blend tuned OOF > 0.969154 (best honest OOF, script 32).
"""
# ruff: noqa: E402
...
EXISTING_SEEDS = [42, 43, 44]  # trained in script 36
...
LGBM_3SEED_OOF = PROJECT_ROOT / "experiments" / "36_spatial_1500trees_oof_probabilities.npy"
LGBM_3SEED_TEST = PROJECT_ROOT / "experiments" / "36_spatial_1500trees_test_probabilities.npy"
...
NEW_SEEDS_OOF = PROJECT_ROOT / "experiments" / "38_spatial_1500trees_extra_seeds_oof.npy"
NEW_SEEDS_TEST = PROJECT_ROOT / "experiments" / "38_spatial_1500trees_extra_seeds_test.npy"
EXPERIMENT = PROJECT_ROOT / "experiments" / "38_spatial_5seed_1500trees.json"
SUBMISSION = PROJECT_ROOT / "submissions" / "38_spatial_5seed_1500trees.csv"
...
def run_new_seeds(X, y, X_test, cat_cols):
    ...
    for seed in NEW_SEEDS:
        ...
        for fold, (tr, va) in enumerate(skf.split(X, y), 1):
            ...
            oof[va] += model.predict_proba(X.iloc[va]) / n
            test += model.predict_proba(X_test) / (n * CV_N_SPLITS)
    return oof, test


def main() -> int:
    if not LGBM_3SEED_OOF.exists():
        raise FileNotFoundError(
            "Run scripts/36_spatial_more_trees.py first to generate 3-seed 1500-tree OOF."
        )
    ...
    if NEW_SEEDS_OOF.exists() and NEW_SEEDS_TEST.exists():
        print("reusing cached extra-seed 1500-tree OOF")
        new_oof = np.load(NEW_SEEDS_OOF)
        new_test = np.load(NEW_SEEDS_TEST)
    else:
        new_oof, new_test = run_new_seeds(X, y, X_test, cat_cols)
        np.save(NEW_SEEDS_OOF, new_oof)
        np.save(NEW_SEEDS_TEST, new_test)

    lgbm_3seed_oof = np.load(LGBM_3SEED_OOF)
    lgbm_3seed_test = np.load(LGBM_3SEED_TEST)

    n_exist = len(EXISTING_SEEDS)
    n_new = len(NEW_SEEDS)
    lgbm_5seed_oof = (n_exist * lgbm_3seed_oof + n_new * new_oof) / (n_exist + n_new)
    lgbm_5seed_test = (n_exist * lgbm_3seed_test + n_new * new_test) / (n_exist + n_new)
    ...
    for w in np.linspace(0, 1, 21):
        blend = w * lgbm_5seed_oof + (1 - w) * xgb_oof
        mult, score = search_class_multipliers(y, blend)
        if best is None or score > best["score"]:
            best = {"w_lgbm": float(w), "score": score, "mult": mult}

    blend_test = best["w_lgbm"] * lgbm_5seed_test + (1 - best["w_lgbm"]) * xgb_test
    mult = best["mult"]
    recalls = per_class_recall(y, (best["w_lgbm"] * lgbm_5seed_oof + (1 - best["w_lgbm"]) * xgb_oof * mult).argmax(1), CLASS_LABELS)

    print("\n================ 5-SEED 1500-TREE SPATIAL BLEND ================")
