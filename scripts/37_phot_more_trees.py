"""Spatial+photometric LightGBM with more trees (n_estimators=1500).

The single-fold comparison showed both spatial-only and spatial+photometric
models hitting max iterations at n_estimators=900. Increasing capacity
may allow the photometric model to surpass spatial-only.

Requires: scripts 15 and 28 to have been run first (for cached features).
Acceptance gate: blend tuned OOF > 0.969071.
"""
# ruff: noqa: E402
...
LGBM_PARAMS = {
    ...
    "n_estimators": 1500,  # increased from 900 in script 28
    "learning_rate": 0.04,
    "num_leaves": 63,
...
LGBM_OOF_OUT = PROJECT_ROOT / "experiments" / "37_phot_1500trees_oof_probabilities.npy"
LGBM_TEST_OUT = PROJECT_ROOT / "experiments" / "37_phot_1500trees_test_probabilities.npy"
EXPERIMENT = PROJECT_ROOT / "experiments" / "37_phot_more_trees.json"
SUBMISSION = PROJECT_ROOT / "submissions" / "37_phot_more_trees.csv"
...
def main() -> int:
    for req in (SP_TRAIN, PHOT_TRAIN):
        if not req.exists():
            raise FileNotFoundError(f"Required file {req.name} not found.")
    ...
    if LGBM_OOF_OUT.exists() and LGBM_TEST_OUT.exists():
        print("reusing cached 1500-tree photometric LGBM")
