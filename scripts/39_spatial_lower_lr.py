"""Spatial LGBM with lower learning rate (lr=0.02, n_estimators=3000).

Lower learning rate with proportionally more trees gives the same total
"learning budget" as lr=0.04/n_est=1500, but with finer-grained steps that
often generalise better. Equivalent of lr=0.02 is lr=0.04 × 0.5, so n_est
should be 2× = 3000 to cover the same learning depth.
...
CV_SEEDS = [42, 43, 44]
CV_N_SPLITS = 5
INCUMBENT_OOF = 0.969154

LGBM_PARAMS = {
    "objective": "multiclass",
    "class_weight": "balanced",
    "n_estimators": 3000,      # 2× vs 1500
    "learning_rate": 0.02,     # half of 0.04
    "num_leaves": 63,
...
LGBM_OOF_OUT = PROJECT_ROOT / "experiments" / "39_spatial_lowlr_oof_probabilities.npy"
LGBM_TEST_OUT = PROJECT_ROOT / "experiments" / "39_spatial_lowlr_test_probabilities.npy"
EXPERIMENT = PROJECT_ROOT / "experiments" / "39_spatial_lower_lr.json"
SUBMISSION = PROJECT_ROOT / "submissions" / "39_spatial_lower_lr.csv"
...
def main() -> int:
    ...
    if LGBM_OOF_OUT.exists() and LGBM_TEST_OUT.exists():
        print("reusing cached low-lr spatial LGBM")
